...
def sky_sub(path, obj_name, color):
    import glob
    import os
    if not os.path.exists(path + '/sky_subed_'+color):
        os.mkdir(path + '/sky_subed_'+color)
    p = sorted(glob.glob(path + '/'+color+'_pp/pp*.fits'))
    ...
    def sky(p, m, i):
        ...
        fits.writeto(path +'/sky_subed_'+color+'/pp0' + obj_name + str(n)+'_'+color+'.fits',subed , header=hdr, overwrite=True)
        bar1.update(i)
    ray.get([sky.remote(p,m,i) for i in range(len(p))])
    ray.shutdown()
    bar1.finish()
